...
|---|---:|---:|---:|---:|
| `1x` | 0.2587 | **+6.9%** | 0.1955 | −19.2% |
| `2x` | 0.1827 | −24.5% | 0.1339 | −44.7% |
| `4x` | 0.1369 | −43.4% | 0.1018 | −57.9% |
| `8x` | 0.1055 | −56.4% | 0.0821 | −66.1% |

- 同一批样本上冻结证书为 0.2420，**不可靠的"删包络"上限为 −44.0%**。
- **那个 −44% 上限是假象。** 老老实实为尾部付钱之后变成 `+6.9%`——比冻结证书还差；而且在任何一档都没能反超简单的换不等式（`8x` 时 `−56.4%` 对 `−66.1%`）。
- 构造本身**是可靠的**：`192` 次证书拟合零覆盖违规。可靠和有用是两件事，报告分开写。
- 机制已经精确：`tau` 取自独立的另一半，所以**经验尾部质量在 48 条记录上全部恰好为 0**，整个代价落在 Cauchy-Schwarz 偏置 `sqrt(V_x·p)` 上（约 `0.0266`–`0.0542`），与冻结半径（`0.0485`）**相当**而不是更大——`H5` 预测它会更大，被否定；损失是两项相加，不是哪一项独大。
