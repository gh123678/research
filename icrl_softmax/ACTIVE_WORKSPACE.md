...
- `841` 次发出更新全部有效：`0` 分量退化、`0` 证书违规、`0` 缺少理由的弃权。
...
- 32 步内没有任何一步种群回升（`H4` 通过），平均增益逐级衰减（`H5` 通过）。
...
- 这是**网络路线**的限制，与迭代能走多远是两件事：FP-HORIZON-001 里 numpy 到第 32 步还在以 `3.8e-7` 的余量发出，而在那个量级上网络根本无法被信任做出同一个判断。

以上同样是受限小环境与冻结协议下的**初步结果**，且仍为同一执行者自检。numpy 路线测到 32 步；网络路线测到 12 步，更远处是算术而不是证书在起作用。
...
- **答案：迭代是几何收敛的。** 一条长轨迹上，每一步收掉剩余间隙的**约 45%，从第 4 步稳定到第 32 步**。所以绝对增益变小是间隙变小的结果，不是迭代停止做功的证据。`38` 条长轨迹（≥24 步）平均收掉初始次优性的 **`99.97%`**。
- **因此尾部更新算数。** "`3.8e-7` 够不够大"这个问题有了干净的回答：它是当时剩余量的约 45%。
...
- 全程带着政策类限制：策略类是 `pi_min = 0.15` 的相对 softmax 族而不是整个单纯形，所以到 `v*` 的间隙本就不会归零；本任务**不**把它拆成"策略类的一份"和"迭代的一份"。
...
- **可证明下界每步衰减 `6.54` 倍**，从第 1 步的 `3.165e-04` 掉到第 32 步的 `2.164e-16`，在**第 18 步**穿过实测地板、**第 26 步**穿过机器 epsilon。迭代照旧跑到 32 步。
