"""FP-NET-BOUND-001 evaluator: the attention network under the tightened certificate.

Hypotheses and bands are registered in `docs/research_tasks/FP-NET-BOUND-001.md`
before this run. Eight cells per record-route, all sharing each step's
certification batch while evolving independently:

    producer in {numpy, network}
    route    in {expected_exact, expected_finite}     (selects the array route AND the network)
    lever    in {frozen, L12M(0.05)}

The point of the task is that the certificate bounds the error of **whatever
producer feeds it**, so the audit compares `E_Q` against the realized
`||Qhat - Q^pi||_inf` of THAT cell's own `Qhat` -- numpy's error is never used as
evidence about the network, or the reverse.

    python evaluate_fp_net_bound_001.py --output-dir results/FP-NET-BOUND-001/claude/fresh_K16
"""
...
NEW_FILES = ("fixed_policy_tight_certificate.py", "evaluate_fp_net_bound_001.py")
...
def run() -> dict[str, Any]:
    ...
    for mixing in fs.MIXING:
        for task_index in FRESH_TASKS:
            ...
            for route in ROUTES:
                ...
                for step in range(1, HORIZON + 1):
                    ...
                    for cell in CELLS:
                        ...
                        # recomputed per cell on purpose: each cell has its own
                        # policy chain, so q_hat is not shareable across cells.
                        q_hat = np.asarray(
                ...
                routes[route] = {
                    cell: {
                        ...
                        "emitted_steps": sum(
                            1 for e in state[cell]["steps"] if e["emitted"]
                        ),
            ...
            records.append(
                {"task_id": TASK_ID, "mixing": float(mixing),
                 "task_index": int(task_index), "routes": routes}
            )
            print(
    ...
    (args.output_dir / "environment.json").write_text(
        json.dumps(
            {"python": sys.version, "platform": platform.platform(),
             "numpy": np.__version__, "torch": _torch_version()},
            indent=2, sort_keys=True,
        ),
        encoding="utf-8",
    )
