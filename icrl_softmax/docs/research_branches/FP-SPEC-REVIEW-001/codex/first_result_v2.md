# Revised independent result (v2) — FP-SPEC-REVIEW-001 / Codex

Date: 2026-09-15. Task v1.0; common baseline 16ee0f652cde56b4f2ef6e3f1a43b58818bb0108.
Status: revised after reciprocal review of the sealed first result. The original remains archived as first_result.md; the derivations and scientific findings are unchanged.
...
- model.py lines 888–1101; imported/called helpers included. Baseline Git blob of model.py: 947542e60ff27aca5c4670982d9bf3fd966d2b23.
...
Code:
- model.py:961–971 creates current-pair equality mask and singleton softmax.
...
- model.py:984–990 reads from one frozen memory and forms the signed residual linearly.
- model.py:992–1011 creates transition match, visited flag, null support, column-normalized writer, then adds one synchronous update.
...
Code model.py:1065–1088 realizes these three matrices: two torch.softmax calls and an explicit exp/normalizer writer algebraically equal to column softmax. Positive finite policy entries and safe finite sharpness are preconditions for the finite-score description; frozen sharpness 8/CPU satisfies the audited scope. Boolean equality is used as a numeric score feature; it does not set support to zero or invoke a visited gate.
...
Labels apply to the named comparison: **exact correspondence**, **explicit adaptation**, **mismatch**, **unresolved**. A local operator can match its specification while being an adaptation of an external architecture.

| Dimension | Exact class | Finite class | Classification and comparator |
|---|---|---|---|
| Tokens/multiplicity | Canonical Q per pair; repeated transition samples; null writer | Canonical Q; repeated samples; no null writer | exact correspondence to local contract; explicit adaptation from both papers |
| Score/support | Singleton current; successor state selection; visited-group writer | ζ state score + log π, ξ pair score, τ writer score; full candidate support | exact correspondence to local operator specification; explicit adaptation from external architectures |
| Normalization/axis | Memory-source normalization; transition/null-source normalization | Read/successor dim=1; writer column normalization | exact correspondence at operator level |
| Values/sign | Signed linear residual, no clipping | Signed residual using leaky reads | exact correspondence to each local operator; explicit adaptation from full prompt/FFN representation |
| Timing/memory | Freeze Q, read all, add once, reconstruct scratch | Same | exact correspondence to local contract; explicit adaptation from Xie's TSM |
| Unvisited | Null enforces zero | Uniform mean residual, potentially nonzero | exact correspondence to respective local contract; explicit adaptation of finite route from old gate-assisted manuscript |
| Successor | Known-policy expectation | Conditional policy expectation plus inter-state leakage | explicit adaptation from sampled SARSA and Xie trajectory successor |
| Policy/parameters | π supplied, no trainable tensors | π supplied in log scores, fixed sharpness | exact correspondence to local contract; mismatch if claimed to reproduce Liang training architecture |
| Finite/exact | Exact support routing | Finite scores, full support, biased reads possible | exact correspondence to local contract; mismatch if treated as universally equal operators |
| Dimension/support | Complete known Q domain, routed support | Leakage depends on S,m,counts,sharpness | exact correspondence to local contract; universal dimension conclusions unresolved by this audit |
| Outer loop | Q/diagnostics only | Q/diagnostics only | exact correspondence to local estimator scope; mismatch if described as internally certifying/updating policy |
| Action-expectation helper | Direct sparse probability construction equals masked log-softmax on valid inputs | Uses its own finite softmax, not this helper | exact correspondence of exact operator; explicit adaptation in executed computational graph |

In the finite WRITER, equality logits are themselves feature-inner-product logits: set φ(x)=sqrt(τ)e_x to get <φ(x),φ(y)>=τ1{x=y}. Thus this kernel is an exact special case of Xie's score form on pair identifiers. The broader architecture still differs: canonical Q retrieval and the altered successor/current residual are not Xie's trajectory-memory/TSM construction.
...
Severity scale: P1 = a claim that could materially change scientific interpretation if used without its conditions; P2 = attribution or execution-description error needing correction. Finding IDs are the numbered items below; multiple findings may share a severity.
...
Claude's first reciprocal review passed the science and requested documentary corrections; this revision tightens the cited code ranges, explicitly classifies each comparison, defines severity, and adds the helper and finite-kernel correspondence. Awaiting confirmation of those corrections.
