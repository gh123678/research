"""FP-COMPOSE-002 / claude: replay verifier for the K=12 bundle.

Carried over from FP-COMPOSE-001 with the method unchanged. It reads chains, chain_length,
horizon, seed, salt and delta_step FROM THE BUNDLE, so one program serves either protocol.

NOTE ON INDEPENDENCE: this verifier is written by the SAME AUTHOR as the evaluator it
checks, and in this task the author also drafted the task sheet. It is therefore a
reproduction check, not independent verification, and it does not raise the evidence grade.
The one discipline this task does have is that the prediction was frozen before the data.

Rebuilds a sealed bundle from the frozen protocol and compares every
