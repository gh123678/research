"""FP-COMPOSE-001 / claude: network x perstate x L12.

Independently written control flow, certificate scalar and per-state decision, per
task sheet section 8. It READS (never modifies) the baseline modules that define the
objects under test: model.py's attention networks, the array routes, the family MDP
generator, the training batch, the dimension-generic sampler and the first-visit
extractor. The L12 scalar (envelope, radius, E_Q), the candidate grid, the row-wise
per-state decision and the audit solve are implemented here rather than imported.

Frozen protocol (task sheet section 3): F1/F2, K=4, 16384 chains x 64,
salt 90417, seed vector [20260911, 90417, round(100*mixing), task_index, step],
routes expected_exact/expected_finite, first-visit retention, min_visits 2000,
delta_total 0.05, delta_dir = delta_step/(S*A), eta grid (1,.5,.2,.1,.05,.02,.01).

    python evaluate.py --output-dir <results/FP-COMPOSE-001/claude/formal>
"""
...
TASK_ID = "FP-COMPOSE-001"
ACTOR = "claude"
BASELINE = "d175379692b8313958032cf1ba8d110df705bcdc"
SEED = 20260911
SALT = 90417
LAYERS = 160
ALPHA = 0.65
...
# file. The task sheet is untracked in the authoring worktree, so its content hash is the
# only integrity anchor available for it.
TASK_SHEET = (PROJECT / "results" / "FP-COMPOSE-001" / "codex_worktree" /
              "icrl_softmax" / "docs" / "research_tasks" / "FP-COMPOSE-001.md")
CALLED_CODE = ("model.py", "fixed_policy_expected_sarsa.py",
               ...
               "docs/research_branches/FP-COMPOSE-001/claude/evaluate.py")
