# Fixed-policy Expected SARSA with certified relative-softmax improvement: main-route theory

Task: `FP-ESARSA-001` (v1.1, `ACTIVE`).
Route: `claude/FP-ESARSA-001` (Claude main route under the user ruling of
2026-09-11).
Scientific/code baseline: `9d0994f03e4a659787c74df4660cdcab1d398c1c`.
Common activation commit: `e2859f2` on `codex/FP-ESARSA-001`.

This document is the Claude main-route consolidated theory. It was derived
before implementation and before any smoke or formal output was inspected; no
formula here may be repaired after empirical output is seen. Claims are
organized against hypotheses H1-H8 of the task definition. The route record
`docs/research_branches/FP-ESARSA-001/claude/theory.md` carries the same
mathematical development with the route provenance and verifier mapping.
