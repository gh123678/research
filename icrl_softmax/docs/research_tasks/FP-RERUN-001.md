...
## 记录区（续）：用户裁决（2026-09-14）

用户就第一轮审阅（`results/FP-L12S-REVIEW-001/`）提出的三项作出裁决：

- **（一）边界裁决：保留 A，暂不启动 B。** 撤回保证、保留测量、收窄结论即可。**暂缓传播是研究优先级决定，不是证明这条方向不可能成功**；今后若重启，必须**从新构造和预审开始**。
- **（二）`H5`：按原交付要求验收，新增阈值改为事后观察。** 选项 **(i)**。逐项核对原要求是否交齐，**齐全才记为"PASS：交付要求满足"**；缺项则列出，不能直接补判通过。记录：分析器曾把**交付要求**替换成**收益阈值**；该阈值**不属于预登记假设**，其结果仅作**事后描述**。
  - 本次逐项核对结果：**(b) 单独运行数据量 ✓（§2）**、**(c) 同总成本截断 ✓（§4）**、**(a) 每格每步预算 ✗ —— 原报告正文缺失**（仅在封存 `items_per_step` 与分析器输出中）。已补齐并**在正文标注为事后补入**，不视为原报告已具备。据此记 **PASS：交付要求满足**。
- **（三）`H6`：作废。** 选项 **(i)**，标为 **INVALID：任务条款冲突且判据不能检验所称目标**；保留原条文与历史结果供追溯。**理由更正**（用户指出）：不能称其本意已由 `H2` 承担——`FP-RERUN-001` 的 `H2` 检验的是**收益提升**；"`16k` 档复现 FP-XRULE-002"是 **`FP-COST-001` 的 `H2`**，核对的是**另一组运行**，**不能**证明本任务与旧 `L12M` 运行的批次一致。
  - 记录为：**撤销 `H6` 的验收效力；原测量证据保留；两次运行的批次一致性仍须单独核查，不能宣称已被替代验证。**
