...
- 2026-09-13：执行完成（first-n 协议）。D1–D6 全部交付。**其保证声明随后因审读 #1 撤回**（见下）。
- 2026-09-13：**审读 #1（用户指定独立审读方）裁定 OBJECTION**：引理 A 被精确反例证伪（计数依赖残差序列；归纳步的 `{τ_n<∞}` 非 `F_{τ_{n-1}}` 可测）。引理 B/C/D、定理 2 适应性机制、风险账务经确认正确；并确认我方发现的 `√2` 缺陷。修复由审读给出：**引理 A'（每链每对首访一个样本）**。
- 2026-09-13：按 A' 修复并重跑（`*_fv` 档）：step1 16k mp 22/48、64k mp 38/48、multi K=12 mp 76 步；验证脚本 PASS；CERTCHECK/NETX/EARLYSTOP 均在 A' 下重跑。
- 2026-09-13：**审读 #2 裁定**：数学部分**全部 PASS**——引理 A' 独立重证为真；定理 1'/2、§7 对照臂 PASS；封存 v2 头条数字由审读方独立重放，8 个 rung **零差异**；覆盖实验 6 环境 × 150 重复：**0/900 记录级、0/10800 逐对级失效**（名义 5%/0.83%），`N_x` 与取值相关性无漂移证据。**但提出流程 OBJECTION**：任务单预登记的停止条件要求转入"oracle 核直接采样"替代协议，实际采用了第三个、未预登记的协议（首访复制，由审读方提出、又由审读方验证），方法变更未经用户裁决。**受影响工作按 AGENTS.md §4 进入 BLOCKED_BY_OBJECTION，等待用户裁决；审读方推荐选项 A（追认 A' 为主协议 + oracle 核臂登记为第二确认臂 + 修正"validity costs power"表述）。**
- 2026-09-13：审读 #2 的两处文档不一致已修（本模块顶部横幅与推导 §9 实现表的旧臂指向）；"validity costs power" 表述已按逐档实际差额改写（16k 档为真实样本量代价；64k 档的 38 vs 36 由 2 条弃权差异解释，与有效性无关）。
- **审读 #2 的实质发现（影响下一步优先级）**：`realized/E_Q` 中位数仅 0.10–0.19（最松处大 37 倍），即 **E_Q 平均比真实误差大 5–10 倍**；因此**当前瓶颈是界的保守度，不是数据量**（加数据的边际收益为 `1/√n`）。oracle 核臂（预登记 fallback，审读方运行）发出 41/48、与首访一致 45/48（3 条不一致全部是"首访弃权、oracle 发出"，方向与"首访有偏"相反），且数据量少 5.3×——但它需要转移核，不能替代 A' 回答"只用行为数据"的核心问题，可作为常设第二臂（待裁决）。
- **用户裁决（2026-09-13）= 选项 A：追认。** ① **引理 A'（每链每对首访一个样本）为本任务主协议**；② 任务单预登记的 fallback（从 `P(·|s,a)` 逐对 iid 直接采样，oracle 转移核）**登记为第二确认臂**并常设保留，作为 A' 的独立构造——已一级实现（`fp_sample_vectorised_batch.kernel_batch`、`evaluate_fp_certfix_001.py --extraction oracle_kernel`）并封存 `results/FP-CERTFIX-001/claude/step1_ok_c64k/`；③ 报告表述按审读 #2 的实际差额改正；④ 受影响工作**解除 `BLOCKED_BY_OBJECTION`**，任务回到 `ACTIVE`。裁决只追认协议与产物，不改变"结论仍为初步结果"：**独立性的真正闭合仍需要一个不属于本执行者的第三方**（审读 #1/#2 与 A' 的提议、实现、验证同源）。
- 未决（与本任务无关但挂着）：FP-TIGHT-001 `bernstein` 臂与 FP-RANGE-001 `data_range` 阶梯在 `√2` 修正后尚未重测。
