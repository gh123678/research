...
**术语（预审第 3 轮的修复：杜绝"路线"一词的多义）**：

| 术语 | 含义 | 取值 |
|---|---|---|
| **actor（执行者）** | 独立实现并执行的一方 | `gpt`、`claude` |
| **algorithm（估计路线）** | 数组生产者的算法变体 | `expected_exact`、`expected_finite` |
| **arm（规则臂）** | 判决规则 | `conj_n16k`、`conj_n64k`、`perstate_n16k` |
| **producer（生产者）** | `Q̂` 的来源 | `numpy`（float64）、`network`（float32 规格算子） |
| **record（记录）** | 统计单元 | `(environment, algorithm)`；每个 actor 有 **48** 条 |

**一条轨迹** = 一个 `(actor, record, arm, producer)`。每条记录含 `3 臂 × 2 生产者 = 6` 条轨迹。
**本任务单此后不再单独使用"路线"一词**；凡统计与验收都按上述四轴点名。

**配对胜负**（同记录、同 actor、同 producer 内两臂比 `closure`）：`win` ⟺ `closure_C > closure_A + 1e-9`；
`loss` ⟺ `closure_C < closure_A − 1e-9`；否则 `tie`。`1e-9` 为冻结平局容差。
**救出**：一条记录满足"A 臂在全部 12 步从未发出，且 C 臂至少发出一次"。
**首步发出**：该臂该记录第 1 步 `emitted = true`。

**预登记假设（任一不成立即按 §6 记录为 FALSIFIED 并原样报告，不调参补救）**：

- **G1（规格自洽）**：两 actor 各自的 spec 实现在**全部 24 环境 × 12 步**上产出**逐位一致**的
  环境、训练批、认证批与首访压缩（内容哈希全等）。
- **G2（基线符合）**：spec 产出与**基线参考实现**（只作 oracle 使用，见 §4）的对应产物逐位一致。
  不一致时按 §2.1 的**封闭决策树**处理。
- **P1（安全）**：对**每个 (actor, record, arm, producer) 轨迹**的每个实际评估步，
  `E_Q ≥` 该格实测 `‖Q̂−Q^π‖∞`（独立审计通道求解）；每个发出步逐分量退化 `min Δv ≥ −1e-12`；
  无错误发出。0 违规只是失败检测，不是概率证明。
- **P2（跨 actor 判决一致）**：48 记录 × 3 臂 × 2 生产者的**发出序列、选中 η（含"不更新"标记）、
  弃权原因序列**在两 actor 间**逐位一致**。判决是离散量，**必须逐位一致，不设容差**。
- **P3（跨 actor 数值一致）**：同一 `(algorithm, arm, producer, record, step)` 在两 actor 间：
  numpy 路径 `|Δ| ≤ 1e-9`（绝对）；network 路径（float32）`|Δ| ≤ 1e-4`（绝对）；`E_Q` 差 ≤ `1e-9`。
- **R1（限定结论复现；分格验收，预审第 3 轮的修复）**：
  **对每个 `(actor, producer)` 分别验收，共 `2 actor × 2 producer = 4` 格；每格的统计在其
  48 条 `(environment, algorithm)` 记录上计算。不先对 producer 取均值，不存在"合并公式"。**
  每格都必须满足以下全部条件：
  - **`mean(closure_C) − mean(closure_A) ≥ 0.40`**（`closure_x` 为该格 48 条记录上该臂的
    `fraction_gap_closed` 的算术平均；`0.40` 为分数单位，即 40 个百分点。封存对照：`0.744 − 0.179 = 0.565`）；
  ...
  `win 46 / loss 0`、救出 `33`。
  **两 producer 结果不同的情形**：这本身就是 R2 的判决层问题（R2 要求逐位一致）——
  若两 producer 的判决序列不一致，R2 先记 FALSIFIED；R1 因按 producer 分格验收，**不存在**"取哪一个"的问题。
- **R2（生产者对应，每 actor 各自）**：每 `(record, arm)` 上，**network 与 numpy 的判决序列逐位一致**
  （两生产者的 `Q̂` 差按 P3 的网络容差报告，不要求按位相等）。
- **R3（对 v2 封存的一致性）**：每 actor 的 **numpy producer**（v2 封存没有 network producer，
  故只比对 numpy）与 v2 封存包逐字段比对（字段与序列化格式见 §6.1）：
  判决类字段**逐位一致**；`e_q`、`min_lb` 差 ≤ `1e-9`；臂级汇总（闭合、收益、初始量）差 ≤ `1e-6`。
- **X1（最终交叉）**：两 actor 的**每个 `(producer)` 格的结论表**逐格一致（判决）或落在 P3 容差内（数值）。
...
| algorithm | `expected_exact`、`expected_finite`；24 环境 × 2 algorithm = **48 条记录** |
...
- **高精度复核（自足要求，不指向任何既有文件）**：每 actor 须**自己实现**对受影响点的独立高精度重算——
...
- 每 actor：288 条轨迹（48 记录 × 3 臂 × 2 生产者）× ≤12 步；网络前向至多 `3456` 次（4×3 族）；
  认证采样 `576` 批 × `4,194,304` 项 ≈ `2.42e9` 项。单 actor 估计 **≤ 2 小时**（CPU 单进程）。
- **两 actor 顺序执行**（防内存竞争且保持盲态）；超上限则保存已完成与待办，不降低样本量/链数/步数。
...
- 若一方因额度/可用性不可用，**不得**由同一 actor 承担两条执行路线——那正是本任务要消除的结构。
...
**它进入训练批的内容哈希**（与基线封存格式一致），但**本任务的 numpy 与 network 生产者都不读取它**；
...
任何记录若出现该字符串或其它未列原因 → **协议异常，停止该 actor 的生产运行并记录**。
...
**不读认证批**。生产者**无发散守卫**；若任一层 `max|q| > VALUE_BOUND=5`，**停止该轨迹并记为协议异常**
（这是协议级停止，不是静默发散）。
...
每（algorithm, arm, producer, record）轨迹：`emitted`、`states_updated`、选中 `η`（含标记）、`E_Q`、逐步 `v` 链、
`total_gain = Σv_final − Σv_initial`、`closure = total_gain/initial_suboptimality`
（`initial_suboptimality = Σv* − Σv^π0`；若 `≤ 0` 则记 `1.0`——本族未发生）。
**环境是统计单元**；同一 environment 内两 algorithm 配对比较。配对比较定义见 §2 冻结定义（win/loss/tie，`1e-9` 容差）。
...
- 执行还需 GPT 跑 `codex/FP-INDEP-001`；**两个 actor 的路线绝不由同一 actor 承担**。

## 13. 第三轮预审记录（v3 → OBJECTION → v4）

- 2026-09-15：**第三轮预审 OBJECTION**（确认前两轮十项已解决、§2.1 五终态与 §6.1 可往返序列化闭合可执行；
  剩一项维度定义缺口）：
  - **R1 的统计维度未冻结**：每条记录同时有 `numpy` 与 `network` 两个生产者，而 R1 只说"48 条记录"的均值，
    未说清两 producer 是分别验收还是先合并、结果不同时取哪一个；且"每路线"一词在 actor 与 algorithm 之间多义。
  - **v4 修复**：§2 顶部新增**术语表**（actor / algorithm / arm / producer / record），
    声明此后不再单独使用"路线"一词，并把全文残留的多义用法逐处替换；
    R1 改为**对每个 `(actor, producer)` 分格验收（2×2=4 格），每格统计在其 48 条 `(environment, algorithm)` 记录上计算，
    明确不先对 producer 取均值、不存在合并公式**；两 producer 判决不同的情形由 R2 先行判 FALSIFIED，
    R1 因此不存在"取哪一个"的问题；R3 明确只比对 **numpy producer**（v2 封存没有 network producer）。
